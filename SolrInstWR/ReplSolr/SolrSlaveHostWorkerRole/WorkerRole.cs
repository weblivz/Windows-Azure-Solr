        ...
        private void StartSolr()
        {
            try
            {
                ...
                string cmdLineFormat =
                    @"%RoleRoot%\approot\jre{0}\bin\java.exe -Djetty.home={1}SolrStorage\{4} -Dsolr.solr.home={1}SolrStorage\{4}\solr -Djetty.port={2} -Denable.slave=true -DmasterUrl={3} -DdefaultCoreName=slaveCore -jar %RoleRoot%\approot\Solr\{4}\start.jar";

                _masterUrl = HelperLib.Util.GetMasterUrl();
                Log("GetMasterUrl: " + _masterUrl, "Information");

                string cmdLine = String.Format(cmdLineFormat, RoleEnvironment.GetConfigurationSettingValue("JavaVersion"), vhdPath, _port, _masterUrl + "replication", RoleEnvironment.GetConfigurationSettingValue("SolrInstanceName"));
